...
async def _run_race_test(mcp: SandboxMCP, session_id: str, coro_func: Any, mock_runtime: Any, success_check: Any) -> None:
    ...
    # Setup returns
    mock_runtime2.execute.return_value = ExecutionResult(stdout="retry_success", stderr="", exit_code=0, artifacts=[], execution_duration=0.1)
...
async def test_execution_during_reaping_race(mock_factory: Any, mock_runtime: Any) -> None:
    ...
    await _run_race_test(
        mcp,
        "race_exec",
        lambda: mcp.execute_code("race_exec", "python", "pass"),
        mock_runtime,
        check
    )
    await mcp.shutdown()
...
async def test_install_package_during_reaping_race(mock_factory: Any, mock_runtime: Any) -> None:
    ...
    await _run_race_test(
        mcp,
        "race_install",
        lambda: mcp.install_package("race_install", "pkg"),
        mock_runtime,
        check
    )
    await mcp.shutdown()
